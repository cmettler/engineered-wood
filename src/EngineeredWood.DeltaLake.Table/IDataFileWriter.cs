...
{
    /// <summary>Writes <paramref name="batch"/> as a single parquet file at <paramref name="relativePath"/>
    /// (relative to the table root, including any partition subdirectory), and returns the written file's byte
    /// size (stored on the <c>add</c> action). The implementation is responsible for placing the bytes at the
    /// location the table's filesystem maps <paramref name="relativePath"/> to.</summary>
    ValueTask<long> WriteAsync(RecordBatch batch, string relativePath, CancellationToken cancellationToken);
}
