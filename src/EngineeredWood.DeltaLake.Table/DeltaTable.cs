...
public sealed class DeltaTable : IAsyncDisposable, IDisposable
{
            ...
            if (cdfEnabled)
            {
                for (int b = 0; b < deletedRowBatches.Count; b++)
                {
                    ...
                    var cdcAction = await ChangeDataFeed.CdfWriter.WriteAsync(
                        ...
                        cancellationToken, trk?.Item1, trk?.Item2).ConfigureAwait(false);
            ...
            if (cdfEnabled)
            {
                for (int b = 0; b < preimages.Count; b++)
                {
                    ...
                    var cdcAction = await ChangeDataFeed.CdfWriter.WriteAsync(
                        ...
                        addFile.PartitionValues, _options.ParquetWriteOptions,
                        cancellationToken, trk.PreIds, trk.PreVers).ConfigureAwait(false);
                    actions.Add(cdcAction);
                }
                for (int b = 0; b < postimages.Count; b++)
                {
                    ...
                    var cdcAction = await ChangeDataFeed.CdfWriter.WriteAsync(
                        ...
                        addFile.PartitionValues, _options.ParquetWriteOptions,
                        cancellationToken, trk.PostIds, trk.PostVers).ConfigureAwait(false);
                    actions.Add(cdcAction);
                }
            }
        }
        ...
        if (partitionColumns is not { Count: > 0 })
        {
            ...
            [
                await ChangeDataFeed.CdfWriter.WriteAsync(
                    ...
                    cancellationToken, rowIds, rowCommitVersions).ConfigureAwait(false),
            ];
        }
    ...
    /// <summary>
    /// MERGE-ON-READ UPDATE of the rows named by a <see cref="RowSelection"/> — the UPDATE analogue of
    /// <see cref="DeleteRowsAsync(RowSelection, RowDeleteMode, CancellationToken)"/>'s
    /// <see cref="RowDeleteMode.DeletionVector"/>, and the CHEAP shape. Instead of rewriting each affected file,
    /// it deletion-vector-masks the old rows and APPENDS the new ones as a small post-image file, fusing both
    /// into ONE commit. For a small update against a large file that is dramatically less IO than
    /// <see cref="UpdateRowsAsync(RowSelection, Func{string, IReadOnlyList{RecordBatch}, IReadOnlyList{Int64Array}, IReadOnlyList{RecordBatch}}, CancellationToken)"/>,
    /// which must read and rewrite the whole file.
    /// </summary>
    ...
    /// <param name="updater">Receives the MATCHED rows of one file and returns them modified — identical
    /// schema, identical row count.</param>
    /// <remarks>
    /// <para>
    ...
    /// writer). Both are clean errors, never a silent fallback — the caller chooses copy-on-write explicitly
    /// via <see cref="UpdateRowsAsync(RowSelection, Func{string, IReadOnlyList{RecordBatch}, IReadOnlyList{Int64Array}, IReadOnlyList{RecordBatch}}, CancellationToken)"/>.
    /// </para>
    /// <para>
    ...
    /// </summary>
    /// <remarks>
    /// ⚠ INVOCATION GRANULARITY DIFFERS from the copy-on-write
    /// <see cref="UpdateRowsAsync(RowSelection, Func{string, IReadOnlyList{RecordBatch}, IReadOnlyList{Int64Array}, IReadOnlyList{RecordBatch}}, CancellationToken)"/>,
    /// and a keyed caller must not assume otherwise: <paramref name="updater"/> is called once per SOURCE BATCH
    /// that contains a matched row — possibly several times for one file — and sees only the MATCHED rows,
    /// whereas the copy-on-write form is called once per FILE with ALL of its batches. Key your values by
    /// <c>(filePath, position)</c> rather than accumulating state per call.
    /// </remarks>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var dv = await ComputeDvActionsWithEditsAsync(selection, snapshot, cancellationToken)
            .ConfigureAwait(false);
        ...
        var extra = new List<DeltaAction>(dv.Actions);
        if (cdfEnabled)
        {
    ...
    /// column names, same combinability, and <see cref="GetReadSchema"/> reports the result. This read used to
    /// be the ONE that could not ask: it offered <paramref name="sourceRowTrackingOut"/> for the stable
    /// identity and nothing at all for the address, so a host needing the address had to reconstruct it from
    /// outside — which it cannot, since the absolute position is never surfaced. The enum's own argument
    /// applies here unchanged: asking for two kinds costs ONE pass.
    ///
    /// <para><see cref="DeltaRowMetadata.RowTracking"/> yields the same values as
    /// <paramref name="sourceRowTrackingOut"/>, as columns rather than out-params; both are accepted, and
    /// asking for both is not an error.</para></param>
