...
public static class ProtocolVersions
{
    ...
    {
        // These are ENFORCEMENT features a writer must honor when ACTIVE; they are almost always LISTED (not
        // active) because enabling a real feature (columnMapping/deletionVectors/…) upgrades the table to the
        // writer-v7 "table features" protocol, which enumerates the legacy writer-v2/v3 features explicitly. The
        // writer honors them per-table: appendOnly is enforced only when delta.appendOnly=true; invariants /
        // checkConstraints are enforced (or the write is rejected) only when a column/table actually declares
        // an expression — see DeltaTable.HonorWriterFeatures. Listing them here just means "we understand them".
        "appendOnly",
        "invariants",
        "checkConstraints",
        "changeDataFeed",
        // Clustered (liquid-clustering) tables: the feature is advisory LAYOUT — the spec permits plain
        // (unclustered) appends and DML; a later clustering OPTIMIZE reclusters them. The obligations a
        // non-clustering writer has are already met here: the `delta.clustering` domainMetadata (the
        // clustering-columns spec) is preserved through commits AND checkpoints (SnapshotBuilder /
        // CheckpointWriter carry all domains), and `add.clusteringProvider` round-trips (AddFile /
        // ActionSerializer). Writing CLUSTERED files (Hilbert layout + provider tagging) is not implemented.
        "clustering",
        "columnMapping",
        "deletionVectors",
        "domainMetadata",
